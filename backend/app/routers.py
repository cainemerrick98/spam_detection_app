...
from app.custom_transformers import EmailToWordCounterTransformer, WordCounterToVectorTransformer
...
with open(os.path.join(os.getcwd(), 'app/models/preprocess_pipeline.pkl'), 'rb') as pipeline:
    preprocess_pipeline: Pipeline = joblib.load(pipeline)

with open(os.path.join(os.getcwd(), 'app/models/logistic_classifier.pkl'), 'rb') as logistic:
